<script>
  ...
  let colorScale1 = chroma.scale(['#995328', '#e67d3c']).mode('oklch');
  let colorScale2 = chroma.scale(['#cc6230', '#cc6230']).mode('oklch');
  let colorScale3 = chroma.scale(['#e67d3c', '#995328']).mode('oklch');
</script>
    ...
    {#each Array(moduleCountY) as _, yi}
      {#each Array(moduleCountX) as _, xi}
        <g transform="translate({xi * triangleWidth * 4 + getOffset(xi, yi)},{yi * triangleHeight * 6})">
          <g>
            <polygon points={pts(p2)} fill={colorScale1(t)} stroke-width="0"/>
            <polygon points={pts(p1)} fill={colorScale2(t)} stroke-width="0"/>
            <polygon points={pts(p3)} fill={colorScale3(t)} stroke-width="0"/>
          </g>
